...
if (length > 5) {
  let welcome = "Hi 1xcoder, welcome to block scope";
  console.log(welcome);
}
...
```js
localStorage.setItem("name", "1xcoder"); // set an item
...
Here, "1xcoder" is logged after 2 seconds.
...
Here, "1xcoder" is printed every 2 seconds.
...
```js
function start() {
  var name = "1xcoder"; // name is a local variable created by start()
...
start(); // "1xcoder" alert box is displayed
```
...
Here,
The instance of displayName maintains a reference to its lexical environment, within which the variable name exists. For this reason, when fun() is invoked, the variable name remains available for use, and "1xcoder" is passed to alert.
...
[1xcoder's create username debounce
...
Don't forget to look at the [Resources](https://1xcoder.vercel.app/resources) and [Snippets](https://1xcoder.vercel.app/snippets) page where I regularly update cool stuff for developers to make their lives easier.
